...
### 24. `unpivot_data(dynamic_frame: DynamicFrame, unpivot_columns: list, key_column: str, value_column: str) -> DynamicFrame`
Unpivots specified columns to rows.
- **Parameters**:
  - `dynamic_frame`: Glue `DynamicFrame` to transform.
  - `unpivot_columns`: List of columns to unpivot.
  - `key_column`: Name of the key column.
  - `value_column`: Name of the value column.
- **Returns**: Unpivoted `DynamicFrame`.

### 25. `apply_window_function(dynamic_frame: DynamicFrame, partition_by: list, order_by: str, function: str) -> DynamicFrame`
Applies a window function (e.g., rank, row_number) to a `DynamicFrame`.
- **Parameters**:
  - `dynamic_frame`: Glue `DynamicFrame` to transform.
  - `partition_by`: List of columns to partition by.
  - `order_by`: Column to order by.
  - `function`: Window function to apply (`"rank"`, `"row_number"`).
- **Returns**: `DynamicFrame` with the window function applied.

### 26. `encode_categorical_columns(dynamic_frame: DynamicFrame, columns: list) -> DynamicFrame`
Converts categorical columns to numeric representations.
- **Parameters**:
  - `dynamic_frame`: Glue `DynamicFrame` to transform.
  - `columns`: List of categorical columns to encode.
- **Returns**: `DynamicFrame` with encoded columns.

### 27. `impute_missing_values(dynamic_frame: DynamicFrame, columns: list, method: str = "mean") -> DynamicFrame`
Imputes missing values in specified columns using a statistical method.
- **Parameters**:
  - `dynamic_frame`: Glue `DynamicFrame` to transform.
  - `columns`: List of columns to impute.
  - `method`: Method for imputation (`"mean"`, `"median"`).
- **Returns**: `DynamicFrame` with imputed values.

### 28. `bucketize_column(dynamic_frame: DynamicFrame, column: str, splits: list, output_column: str) -> DynamicFrame`
Bucketizes a numerical column into bins.
- **Parameters**:
  - `dynamic_frame`: Glue `DynamicFrame` to transform.
  - `column`: Column to bucketize.
  - `splits`: List of split points for bins.
  - `output_column`: Name of the bucketized column.
- **Returns**: `DynamicFrame` with bucketized columns.

### 29. `concatenate_columns(dynamic_frame: DynamicFrame, columns: list, output_column: str, separator: str = " ") -> DynamicFrame`
Concatenates multiple columns into a single column.
- **Parameters**:
  - `dynamic_frame`: Glue `DynamicFrame` to transform.
  - `columns`: List of columns to concatenate.
  - `output_column`: Name of the new concatenated column.
  - `separator`: Separator between concatenated values.
- **Returns**: `DynamicFrame` with concatenated columns.

### 30. `validate_schema(dynamic_frame: DynamicFrame, expected_schema: dict) -> bool`
Validates if the schema matches an expected structure.
- **Parameters**:
  - `dynamic_frame`: Glue `DynamicFrame` to validate.
  - `expected_schema`: Dictionary mapping column names to expected data types.
- **Returns**: `True` if schema matches, else `False`.
